...
public class DishController {
    ...
    public R<Page> page(int page,int pageSize, String name){
        ...
        List<DishDto> list = records.stream().map((item) -> {
            ...
            BeanUtils.copyProperties(item,dishDto);   // 把普通属性拷贝进去
            Long categoryId = item.getCategoryId();  // 找到分类id
            Category category = categoryService.getById(categoryId);  // 根据id查询分类对象
            if (category != null){
                String categoryName = category.getName();   // 从数据库找到分类的名字
    ...
    public R<List<DishDto>> list(Dish dish) {
        ...
        queryWrapper.eq(Dish::getStatus, 1);      //只查询状态为1（在售）的菜品
        //添加排序条件
